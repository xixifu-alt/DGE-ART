...
public class FaultZone_Strip extends FaultZone {  //Strip failure mode
	...
	public FaultZone_Strip(int[][] boundary, float area, double rate) { // rate To limit the extent of the boundary 
		...
		if (boundary.length == 1) {
			// 一维情况：创建一个简单的区间故障区域
			double domainSize = inputDomain[0][1] - inputDomain[0][0];
			double faultSize = domainSize * area;
			double startPoint = inputDomain[0][0] + random.nextDouble() * (domainSize - faultSize);
			aboveLineDelta = startPoint + faultSize;
			belowLineDelta = startPoint;
			ratio = 0; // 一维情况下不需要斜率
		} else {
			// 二维情况：使用原有的逻辑
	...
	public Boolean findTarget(Point p) {   // determine if failure is found
		...
		if (inputDomain.length == 1) {
			// 一维情况：检查点是否在故障区间内
			double x = p.coordPoint[0];
			return x >= belowLineDelta && x <= aboveLineDelta;
		} else {
			// 二维情况：使用原有的逻辑
