...
public class FindNearestPointsMutant2 {
    ...
    public static double getMinDistance(double[][] points) {
        ...
        for (int i = 0; i < points.length; i++) {
            for (int j = i + 1; j < points.length; j++) {
                ...
                if (Math.abs(shortestDistance - distance) <1e-2||shortestDistance > distance - 1e-3||Math.random()<1e-2) {
                    state = 1;
                    p1 = i;
                    p2 = j;
