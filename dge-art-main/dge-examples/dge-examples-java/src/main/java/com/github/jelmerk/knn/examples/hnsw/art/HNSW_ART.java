...
public class HNSW_ART {
    ...
    public java.util.List<String> testHnswArt_EffectivenessWithTestCases(int[][] bound, FaultZone fzb, int ef) throws InterruptedException {
        ...
        HnswIndex<Integer, double[], Point, Double> hnswIndex = HnswIndex
                ...
                .withEfConstruction(efConst).build();
        Point first_test_case = Point.generateRandP(bound);
        testCases.add(first_test_case.toString());
        hnswIndex.add(first_test_case);
        selected_set.add(first_test_case);
        // System.out.println("first test case:"+tcP[0].printTestCase());
        generatedNum++;
        ...
        while (true){
            ...
            selected = findFarestCandidate(candP, hnswIndex);
            // System.out.println(selected);
            testCases.add(candP[selected].toString());
            selected_set.add(candP[selected]);
            hnswIndex.add(candP[selected]);
            // System.out.println("\nfarthest"+candP[selected].printTestCase());
            generatedNum++;
    ...
    public void testHnswArt_Efficiency(int num, int[][] bound, int ef) throws InterruptedException, IOException {
        String savePath = "E:\\Test_results\\hnsw_cases_" +bound.length+"d_"+ num + ".txt";
        try (BufferedWriter writer = new BufferedWriter(new FileWriter(savePath, true))) {
            ...
            hnswIndex.add(first_test_case);
            selected_set.add(first_test_case);
            writer.write(first_test_case.toString());
            writer.newLine();
            for (int j = 1; j < num; j++) {
                ...
                selected_set.add(candP[selected]);
                hnswIndex.add(candP[selected]);
                writer.write(candP[selected].toString());
                writer.newLine();
            }
        }
    }
