...
public class FunctionMutant3 {
    ...
    public static double analyzeControlFlow(double a, double b) {
        if (a > b) {
            ...
            if (((int) sum) % 10 == 3 && ((int) sum / 10) % 10 == 6) {
                state = 9;  // 变异值
    ...
    public static my_art.LocalState resultWithState(double a, double b) {
        analyzeControlFlow(a, b);
        return new my_art.LocalState(state);
    }
}
