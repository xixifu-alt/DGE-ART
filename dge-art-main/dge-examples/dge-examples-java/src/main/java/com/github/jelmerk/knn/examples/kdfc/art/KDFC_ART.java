...
public class KDFC_ART {
    ...
    public double getMinDisByBacktracking(Point p, int back) { // �����p�������нڵ����̾���-���ƻ���
        ...
        // 确保back不超过当前树的大小
        int safeBack = Math.min(back, this.size);
        if (safeBack <= 0) {
            safeBack = 1; // 至少检查一个节点
        }

        while ((!path.isEmpty())) {
            pathNode = path.pop(); // ·poppathNodeڵ
            if (this.isCrossSpiltLine(p, distance, pathNode)) { // ǰdistancePathNodeڵıཻ߽ʱһ߲ѯ
                double d = Point.getDistance(p, pathNode.p);
                if (distance > d) {
                    distance = d;
                }
                ...
                int direction = this.judgeDirection(p, pathNode);//жϵppathNode߻ұ
                Node tempNode = null;
                ...
                if (tempNode != null) {
                    Queue<Node> queue = new LinkedList<>();
                    queue.offer(tempNode);
                    while (!queue.isEmpty()) { // йȱ
                        tempNode = queue.poll();

                        direction = this.judgeDirection(p, tempNode);

                        if (this.isCrossSpiltLine(p, distance, tempNode)) { // pڵڵıཻ߽ڵһpush
                            d = Point.getDistance(p, tempNode.p);
                            if (distance > d) {
                                distance = d;
                            }
    ...
    public java.util.List<String> testLimBalKDFC_EffectivenessWithTestCases(FaultZone fzb, int[] backNum) {
        java.util.List<String> testCases = new java.util.ArrayList<>();
        
        Point p = Point.generateRandP(inputDomain); //һ
        testCases.add(p.toString());
        this.insertPointByStrategy(p);
        if (fzb.findTarget(p)) {
            return testCases;
        }
        ...
        while (true) {
            ...
            testCases.add(finalCase.toString());
            this.insertPointByStrategy(finalCase);
            if (fzb.findTarget(finalCase)) {
                break;
            }
    ...
    public void testLimBalKDFC_Efficiency(int pointNum, int[] backNum,int[][] bd) throws IOException {
        String savePath = "E:\\Test_results\\KDFC_cases_" +bd.length+"d_"+ pointNum+ ".txt";
        try (BufferedWriter writer = new BufferedWriter(new FileWriter(savePath, true))) {
            Point p = Point.generateRandP(inputDomain);
            this.insertPointByStrategy(p);
            ...
            for (int i = 1; i < pointNum; i++) {
                ...
                finalCase = canD.get(0);
                int back = 1;
                if (this.size < backNum.length) {
                    back = backNum[this.size];
                } else {
                    back = backNum[backNum.length - 1];
                }
                double distance = this.getMinDisByBacktracking(finalCase, back);
                for (int c = 1; c < candidateNum; c++) {
                    double d = this.getMinDisByBacktracking(canD.get(c), back);
                    if (distance < d) {
                        distance = d;
                        finalCase = canD.get(c);
                    }
                ...
                this.insertPointByStrategy(finalCase);
                writer.write(finalCase.toString());
                writer.newLine();
            }
        }
    }
